...
CButtonST::CButtonST()
{
	...
	m_bIsNormal			= FALSE;	//YWJ
	m_bBMPOverlay		= FALSE;	//YWJ
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		//captionRect.OffsetRect(5, (centerRect.Height() - captionRect.Height())/2);
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ((m_bIsCheckBox && bIsPressed))
		byIndex = 0;
	else if (!m_bIsCheckBox && (m_bMouseOnButton && !m_bIsPressed))
		byIndex = 0;
	else if (!m_bIsCheckBox && m_bIsPressed)
...
//
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut, int nBitmapPressed, COLORREF crTransColorPressed)
{
	...
	//YWJ
	if (nBitmapPressed)
		hBitmapPressed = (HBITMAP)::LoadImage(hInstResource, MAKEINTRESOURCE(nBitmapPressed), IMAGE_BITMAP, 0, 0, 0);

	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut, hBitmapPressed, crTransColorPressed);
} // End of SetBitmaps
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut, HBITMAP hBitmapPressed, COLORREF crTransColorPressed)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		if (hBitmapPressed)
		{
			m_csBitmaps[2].hBitmap = hBitmapPressed;
			m_csBitmaps[2].crTransparent = crTransColorPressed;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapPressed, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			// Create mask for bitmap Out
			m_csBitmaps[2].hMask = CreateBitmapMask(hBitmapPressed, m_csBitmaps[2].dwWidth, m_csBitmaps[2].dwHeight, crTransColorPressed);
...
DWORD CButtonST::SetDefaultColors(BOOL bRepaint)
{
	...
	/*
	m_crColors[BTNST_COLOR_BK_IN]		= COLOR_BTN_GREEN;
	m_crColors[BTNST_COLOR_FG_IN]		= COLOR_BTN_BLUE;
	m_crColors[BTNST_COLOR_BK_OUT]		= COLOR_DBLUE;
	m_crColors[BTNST_COLOR_FG_OUT]		= COLOR_BTN_LBLUE;
	m_crColors[BTNST_COLOR_BK_FOCUS]	= COLOR_DBLUE;
	m_crColors[BTNST_COLOR_FG_FOCUS]	= COLOR_BTN_LBLUE;
	*/
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	...
	pDC->RoundRect(pRect,CPoint(12,12));
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	...
	else
	{
		...
		pDC->FrameRect(pRect,&myBrush);
	}
	return BTNST_OK;
} // End of OnDrawBorder

//YWJ
//This function is called if you want to change the button to a normal one.
DWORD CButtonST::SetBtnNormal(BOOL bNormal)
{
	m_bIsNormal = bNormal;
	return BTNST_OK;
}
...
//YWJ
DWORD CButtonST::SetBMPOverlaid(BOOL bOver /* = TRUE */)
{
	m_bBMPOverlay = bOver;
	return BTNST_OK;
}//End of SetBMPOverlaid
